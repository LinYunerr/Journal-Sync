...
saveBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // 异步保存，使用实时更新
        await saveContentWithRealTimeUpdate(saveId, content, currentType, options);
...
// 实时更新保存状态
async function saveContentWithRealTimeUpdate(saveId, content, type, options) {
    try {
...
function createCard(item) {
    ...
    const displayContent = needsTruncate ? content.substring(0, maxLength) : content;
    const cardId = `card-${item.id}`;

    console.log('[createCard] ID:', item.id, 'Content length:', content.length, 'Needs truncate:', needsTruncate);

    let contentHTML = '';
    if (needsTruncate) {
        contentHTML = `
            <div class="timeline-card-content" id="${cardId}-content">
                <span id="${cardId}-preview">${escapeHtml(displayContent)}</span>
                <span id="${cardId}-full" style="display: none;">${escapeHtml(content)}</span>
        ...
        `;
        console.log('[createCard] Generated truncated HTML for:', cardId);
    } else {
        contentHTML = `<div class="timeline-card-content">${escapeHtml(content)}</div>`;
        console.log('[createCard] Generated normal HTML for:', cardId);
    }
